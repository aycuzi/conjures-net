...
app.patch("/api/guides/reorder-folders",requireUser,async(req,res)=>{try{if(!guideRoot(req.user))return res.status(403).json({error:"Only SR Team guide administrators can reorder folders."});const ids=Array.isArray(req.body?.ids)?[...new Set(req.body.ids.map(String))]:[];const folders=await guideRows("guide_folders");if(ids.length!==folders.length||folders.some(folder=>!ids.includes(String(folder.id))))return res.status(400).json({error:"The folder order must include every folder exactly once."});await Promise.all(ids.map((folderId,index)=>api(`/tables/guide_folders?id=${encodeURIComponent(folderId)}`,{method:"PATCH",body:JSON.stringify({sort_order:index,updated_at:new Date().toISOString()})})));res.json({ok:true});}catch(e){console.error("[guides] reorder failed",e);res.status(500).json({error:"Folder order could not be saved."});}});
app.post("/api/guides/folders",requireUser,async(req,res)=>{try{if(!guideRoot(req.user))return res.status(403).json({error:"Only SR Team guide administrators can create folders."});const name=cleanText(req.body?.name,80);if(!name)return res.status(400).json({error:"Folder name is required."});const folders=await guideRows("guide_folders");const row=await api("/tables/guide_folders",{method:"POST",body:JSON.stringify({id:id(),name,description:cleanText(req.body?.description,500),icon_color:cleanHex(req.body?.iconColor),sort_order:folders.length,permissions_json:JSON.stringify(guidePermissions(req.body?.permissions)),created_by_discord_id:req.user.discord_id})});res.status(201).json(Array.isArray(row)?row[0]:row);}catch(e){res.status(500).json({error:"Folder could not be created."});}});
app.patch("/api/guides/folders/:id",requireUser,async(req,res)=>{try{if(!guideRoot(req.user))return res.status(403).json({error:"Only SR Team guide administrators can edit folders."});const rows=await guideRows("guide_folders",`id=${encodeURIComponent(req.params.id)}&limit=1`);if(!rows[0])return res.status(404).json({error:"Folder not found."});const name=cleanText(req.body?.name,80);if(!name)return res.status(400).json({error:"Folder name is required."});const result=await api(`/tables/guide_folders?id=${encodeURIComponent(req.params.id)}`,{method:"PATCH",body:JSON.stringify({name,description:cleanText(req.body?.description,500),icon_color:cleanHex(req.body?.iconColor),permissions_json:JSON.stringify(guidePermissions(req.body?.permissions)),updated_at:new Date().toISOString()})});res.json(Array.isArray(result)?result[0]:result);}catch(e){res.status(500).json({error:"Folder could not be updated."});}});
