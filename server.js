...
app.get("/api/guides",requireUser,async(req,res)=>{try{if(!guideEligible(req.user))return res.status(403).json({error:"You do not have access to Guides."});const [folders,docs]=await Promise.all([guideRows("guide_folders","order=created_at.asc"),guideRows("guide_documents","order=updated_at.desc")]);const result=[];for(const folder of folders){const abilities=folderAbilities(req.user,folder),visibleDocs=docs.filter(d=>d.folder_id===folder.id&&documentAbilities(req.user,folder,d).view).map(d=>({...d,permissions:guidePermissions(d.permissions_json,true),abilities:documentAbilities(req.user,folder,d),content_html:undefined}));if(abilities.view||abilities.create||visibleDocs.length)result.push({...folder,permissions:guidePermissions(folder.permissions_json),abilities,documents:visibleDocs});}res.json({canCreateFolders:guideRoot(req.user),roleOptions:GUIDE_ROLE_NAMES,folders:result});}catch(e){console.error("[guides] list failed",e);res.status(500).json({error:"Guides could not be loaded."});}});
app.post("/api/guides/folders",requireUser,async(req,res)=>{try{if(!guideRoot(req.user))return res.status(403).json({error:"Only SR Team guide administrators can create folders."});const name=cleanText(req.body?.name,80);if(!name)return res.status(400).json({error:"Folder name is required."});const row=await api("/tables/guide_folders",{method:"POST",body:JSON.stringify({id:id(),name,description:cleanText(req.body?.description,500),permissions_json:JSON.stringify(guidePermissions(req.body?.permissions)),created_by_discord_id:req.user.discord_id})});res.status(201).json(row);}catch(e){res.status(500).json({error:"Folder could not be created."});}});
app.patch("/api/guides/folders/:id",requireUser,async(req,res)=>{try{if(!guideRoot(req.user))return res.status(403).json({error:"Only SR Team guide administrators can edit folders."});const rows=await guideRows("guide_folders",`id=${encodeURIComponent(req.params.id)}&limit=1`);if(!rows[0])return res.status(404).json({error:"Folder not found."});const name=cleanText(req.body?.name,80);if(!name)return res.status(400).json({error:"Folder name is required."});const row=await api(`/tables/guide_folders?id=${encodeURIComponent(req.params.id)}`,{method:"PATCH",body:JSON.stringify({name,description:cleanText(req.body?.description,500),permissions_json:JSON.stringify(guidePermissions(req.body?.permissions)),updated_at:new Date().toISOString()})});res.json(row);}catch(e){res.status(500).json({error:"Folder could not be updated."});}});
app.delete("/api/guides/folders/:id",requireUser,async(req,res)=>{try{if(!guideRoot(req.user))return res.status(403).json({error:"Only SR Team guide administrators can delete folders."});await api(`/tables/guide_folders?id=${encodeURIComponent(req.params.id)}`,{method:"DELETE"});res.json({ok:true});}catch(e){res.status(500).json({error:"Folder could not be deleted."});}});
app.get("/api/guides/folders/:id",requireUser,async(req,res)=>{try{if(!guideEligible(req.user))return res.status(403).json({error:"You do not have access to Guides."});const folders=await guideRows("guide_folders",`id=${encodeURIComponent(req.params.id)}&limit=1`),folder=folders[0];if(!folder)return res.status(404).json({error:"Folder not found."});const docs=await guideRows("guide_documents",`folder_id=${encodeURIComponent(folder.id)}&order=updated_at.desc`),visible=docs.filter(d=>documentAbilities(req.user,folder,d).view).map(d=>({...d,content_html:undefined,permissions:guidePermissions(d.permissions_json,true),abilities:documentAbilities(req.user,folder,d)})),abilities=folderAbilities(req.user,folder);if(!abilities.view&&!abilities.create&&!visible.length)return res.status(403).json({error:"You cannot access this folder."});res.json({...folder,permissions:guidePermissions(folder.permissions_json),abilities,roleOptions:GUIDE_ROLE_NAMES,documents:visible});}catch(e){res.status(500).json({error:e.message||"Folder could not be loaded."});}});
app.post("/api/guides/folders/:id/documents",requireUser,async(req,res)=>{try{const folders=await guideRows("guide_folders",`id=${encodeURIComponent(req.params.id)}&limit=1`),folder=folders[0];if(!folder)return res.status(404).json({error:"Folder not found."});if(!folderAbilities(req.user,folder).create)return res.status(403).json({error:"You cannot create documents in this folder."});const title=cleanText(req.body?.title,120);if(!title)return res.status(400).json({error:"Document title is required."});const row=await api("/tables/guide_documents",{method:"POST",body:JSON.stringify({id:id(),folder_id:folder.id,title,content_html:"<p></p>",permissions_json:JSON.stringify(guidePermissions(req.body?.permissions,true)),created_by_discord_id:req.user.discord_id,updated_by_discord_id:req.user.discord_id})});res.status(201).json(row);}catch(e){res.status(500).json({error:"Document could not be created."});}});
...
app.patch("/api/guides/folders/:folderId/documents/:docId",requireUser,async(req,res)=>{try{const [folders,docs]=await Promise.all([guideRows("guide_folders",`id=${encodeURIComponent(req.params.folderId)}&limit=1`),guideRows("guide_documents",`id=${encodeURIComponent(req.params.docId)}&folder_id=${encodeURIComponent(req.params.folderId)}&limit=1`)]),folder=folders[0],doc=docs[0];if(!folder||!doc)return res.status(404).json({error:"Document not found."});const abilities=documentAbilities(req.user,folder,doc),metadata=req.body?.title!==undefined||req.body?.permissions!==undefined;if(metadata&&!abilities.admin)return res.status(403).json({error:"Only document administrators can change its title or permissions."});if(req.body?.contentHtml!==undefined&&!abilities.edit)return res.status(403).json({error:"You cannot edit this document."});const body={updated_at:new Date().toISOString(),updated_by_discord_id:req.user.discord_id,version:Number(doc.version||1)+1};if(req.body?.title!==undefined){body.title=cleanText(req.body.title,120);if(!body.title)return res.status(400).json({error:"Document title is required."});}if(req.body?.permissions!==undefined)body.permissions_json=JSON.stringify(guidePermissions(req.body.permissions,true));if(req.body?.contentHtml!==undefined)body.content_html=cleanGuideHtml(req.body.contentHtml);const row=await api(`/tables/guide_documents?id=${encodeURIComponent(doc.id)}`,{method:"PATCH",body:JSON.stringify(body)});res.json({...row,permissions:guidePermissions(row.permissions_json,true),abilities});}catch(e){res.status(500).json({error:"Document could not be saved."});}});
app.delete("/api/guides/folders/:folderId/documents/:docId",requireUser,async(req,res)=>{try{const [folders,docs]=await Promise.all([guideRows("guide_folders",`id=${encodeURIComponent(req.params.folderId)}&limit=1`),guideRows("guide_documents",`id=${encodeURIComponent(req.params.docId)}&folder_id=${encodeURIComponent(req.params.folderId)}&limit=1`)]),folder=folders[0],doc=docs[0];if(!folder||!doc)return res.status(404).json({error:"Document not found."});if(!documentAbilities(req.user,folder,doc).delete)return res.status(403).json({error:"You cannot delete this document."});await api(`/tables/guide_documents?id=${encodeURIComponent(doc.id)}`,{method:"DELETE"});res.json({ok:true});}catch(e){res.status(500).json({error:"Document could not be deleted."});}});
