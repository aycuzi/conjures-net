...
async function request(url, options = {}) {
  ...
  if (!r.ok) {const error=new Error(data.error || `Request failed (${r.status})`);error.status=r.status;throw error;}
  return data;
}
const avatar = (u) =>
  u.headshot ||
...
app.innerHTML = `<div class="login-shell"><section class="login-card"><img class="mark" src="/images/InitialTypography.png"><div class="portal-title">CONJURES PORTAL</div><p class="muted">Log in with a verified account to access applications and your dashboard.</p>${error ? `<p class="error">${error === "verification_required" ? "You're not verified on Discord. Please ensure you verify and link your Roblox account in the CONJURES Discord server, then you may proceed to log in here." : error === "discord_rate_limited" ? "Discord is temporarily limiting login requests. Please wait a few minutes before trying again, or log in with your linked Roblox account." : error === "site_blacklisted" ? "You are blacklisted from accessing conjures.net." : "Login could not be completed. Please try again."}</p>` : ""}<div class="auth-stack"><a class="auth discord" href="/auth/discord"><img src="/images/Discord.png" alt="">Log in with Discord</a><a class="auth" href="/auth/roblox"><img src="/images/Roblox.png" alt="">Log in with Roblox</a></div></section></div>`;
}
...
const dateOnly=value=>{const date=new Date(value);return Number.isNaN(date.getTime())?"Just now":date.toLocaleDateString("en-US",{month:"2-digit",day:"2-digit",year:"numeric"});};
...
function showDocumentAccessDenied(){modal.classList.remove("hidden");modal.innerHTML='<div class="dialog"><h2>You do not have access to this document</h2><p>This guide has not been shared with your current Roblox rank and Discord roles.</p><div class="actions"><button class="primary" id="close-denied-document">Close</button></div></div>';modal.querySelector("#close-denied-document").onclick=()=>{window.close();setTimeout(()=>{if(!window.closed)location.href="/guides"},150)};}
...
async function guidesV2() {
  ...
  const folderList = () => data.folders.length
    ? `<div id="guide-folder-list" class="guide-folder-grid">${data.folders.map((folder) => `<article class="guide-folder-card" data-folder-id="${folder.id}" style="--folder-color:${esc(folder.icon_color || "#ef4b5f")}">${data.canCreateFolders?'<button class="folder-drag" title="Drag to reorder folder" aria-label="Drag to reorder folder">⠿</button>':''}<div class="folder-icon">📁</div><div><h2>${esc(folder.name)}</h2><p class="muted">${esc(folder.description || "No description provided.")}</p></div><div class="folder-actions"><a class="primary" href="/guides/${folder.id}">Open</a>${folder.abilities.admin ? `<button class="ghost edit-folder-v2" data-id="${folder.id}">Manage</button>` : ""}${folder.abilities.delete ? `<button class="danger delete-folder-v2" data-id="${folder.id}" ${folder.documents.length ? `disabled title="Delete all ${folder.documents.length} documents first"` : ""}>Delete</button>` : ""}</div></article>`).join("")}</div>`
  ...
  if(data.canCreateFolders){const list=document.querySelector("#guide-folder-list");let dragged=null;list?.querySelectorAll(".guide-folder-card").forEach(card=>{const handle=card.querySelector(".folder-drag");let armed=false;handle.onpointerdown=()=>{armed=true;card.draggable=true};handle.onpointerup=()=>{if(!dragged){armed=false;card.draggable=false}};card.addEventListener("dragstart",event=>{if(!armed){event.preventDefault();return}dragged=card;card.classList.add("is-dragging");event.dataTransfer.effectAllowed="move"});card.addEventListener("dragend",async()=>{card.draggable=false;armed=false;card.classList.remove("is-dragging");list.querySelectorAll(".drag-over").forEach(x=>x.classList.remove("drag-over"));const ids=[...list.querySelectorAll(".guide-folder-card")].map(x=>x.dataset.folderId);try{await request("/api/guides/reorder-folders",{method:"PATCH",body:JSON.stringify({ids})});}catch(error){showNotice("Order Not Saved",error.message);setTimeout(()=>guidesV2(),500)}dragged=null});card.addEventListener("dragover",event=>{event.preventDefault();if(!dragged||dragged===card)return;const box=card.getBoundingClientRect(),after=event.clientY>box.top+box.height/2;list.insertBefore(dragged,after?card.nextSibling:card);card.classList.add("drag-over")});card.addEventListener("dragleave",()=>card.classList.remove("drag-over"))})}
...
async function guideFolderV2(folderId) {
  ...
  if(folder.abilities.admin){const list=document.querySelector(".guide-document-list");let dragged=null;list?.querySelectorAll("article[data-doc-id]").forEach(card=>{const handle=card.querySelector(".document-drag");let armed=false;handle.onpointerdown=()=>{armed=true;card.draggable=true};handle.onpointerup=()=>{if(!dragged){armed=false;card.draggable=false}};card.addEventListener("dragstart",event=>{if(!armed){event.preventDefault();return}dragged=card;card.classList.add("is-dragging");event.dataTransfer.effectAllowed="move"});card.addEventListener("dragend",async()=>{card.draggable=false;armed=false;card.classList.remove("is-dragging");list.querySelectorAll(".drag-over").forEach(x=>x.classList.remove("drag-over"));const ids=[...list.querySelectorAll("article[data-doc-id]")].map(x=>x.dataset.docId);try{await request(`/api/guides/folders/${folder.id}/reorder-documents`,{method:"PATCH",body:JSON.stringify({ids})});}catch(error){showNotice("Order Not Saved",error.message);setTimeout(()=>guideFolderV2(folder.id),500)}dragged=null});card.addEventListener("dragover",event=>{event.preventDefault();if(!dragged||dragged===card)return;const box=card.getBoundingClientRect(),after=event.clientY>box.top+box.height/2;list.insertBefore(dragged,after?card.nextSibling:card);card.classList.add("drag-over")});card.addEventListener("dragleave",()=>card.classList.remove("drag-over"))})}
...
async function guideDocumentV2(folderId, docId) {
  ...
  const save = async (silent = false) => {
    ...
    try { const response = await request(`/api/guides/folders/${folderId}/documents/${docId}`, { method: "PATCH", body: JSON.stringify({ contentHtml: cleanEditorHtml() }) }),result=Array.isArray(response)?response[0]:response;if(!result?.content_html)throw new Error("The saved document could not be reloaded.");lastSaved=result.content_html;doc.updated_at=result.updated_at||new Date().toISOString();dirty=false;if(status)status.textContent="Saved just now";if(!silent)showNotice("Changes Saved","Your guide changes have been saved."); }
  ...
  const wire = () => {
    ...
    document.querySelector("#remove-highlight-v2").onclick = () => { restoreRange(); document.execCommand("hiliteColor", false, "transparent"); document.execCommand("backColor", false, "transparent"); const selection=getSelection(),node=selection?.anchorNode?.nodeType===3?selection.anchorNode.parentElement:selection?.anchorNode;node?.closest?.("span,font")?.style?.removeProperty("background-color");changed();area.focus(); };
    ...
    document.querySelector("#selection-remove-highlight").onclick = () => { restoreRange(); document.execCommand("hiliteColor", false, "transparent"); document.execCommand("backColor", false, "transparent"); changed(); area.focus(); };
...
async function route() {
  ...
  try {
    ...
    if(/^\/guides\/[^/]+\/[^/]+\/?$/.test(p)&&(e.status===403||/cannot view|do not have access/i.test(e.message))){showDocumentAccessDenied();return;}
